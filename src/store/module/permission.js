...
const actions ={
    GererateRouter({commit},userPermision){
        return new Promise((resolve,reject)=>{
            let roleName = userPermision.roleName;
            let menuList = userPermision.menuList;
            let accessRouters;
            // if(roleName =="超级管理员"){
            //     accessRouters = asyncRouterMap;
            // }else{
            //    accessRouters = filterAsyncRouter(asyncRouterMap,menuList) 
            // }
            accessRouters = filterAsyncRouter(asyncRouterMap,menuList) 
